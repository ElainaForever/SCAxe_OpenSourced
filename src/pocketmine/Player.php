...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	protected function processLogin(){
		if(!$this->server->isWhitelisted(strtolower($this->getName()))){
			$this->close($this->getLeaveMessage(), "服务器开启了白名单");

			return;
		}elseif($this->server->getNameBans()->isBanned(strtolower($this->getName()))){
			$this->close($this->getLeaveMessage(), "游戏名请不能用Steve，请前往选项中更改！");

			return;
		}elseif($this->server->getIPBans()->isBanned($this->getAddress()) or $this->server->getCIDBans()->isBanned($this->randomClientId)){
			$this->close($this->getLeaveMessage(), "该账号已被封，请加qq群543953483");
